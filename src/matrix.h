   ...
   public:
        ...
        : m_row{row}, m_col{col}
    {
        if (initList.size() != row)
        {
            throw std::invalid_argument(
                "Number of rows in initializer_list does not match row.");
        }
    ...
    Matrix(const std::initializer_list<std::initializer_list<T>>& initList)
    {
        ...
        for (const auto& row : initList)
        {
            ...
            {
                throw std::invalid_argument(
                    "Number of cols in initializer_list are differentS.");
            }
            m_data.emplace_back(row.begin(), row.end());
        }
    }
    ...
    Matrix(const Matrix& matr)
        : m_row{matr.getRow()}, m_col{matr.getColumn()}, m_data{matr.m_data}
    {
    }
    Matrix(Matrix&& matr)
    ...
    bool operator!=(const Matrix<T>& matr) const { return !(*this == matr); }
    inline const std::size_t getColumn() const { return m_col; }
    inline const std::size_t getRow() const { return m_row; }
    inline void setcol(const std::size_t col) { m_col = col; }
    inline void setrow(const std::size_t row) { m_row = row; }
    inline const std::vector<std::vector<T>*>& getData() const
    {
        return m_data;
    }
    inline void setValue(const std::size_t row, const std::size_t col,
                         const T val)
    {
